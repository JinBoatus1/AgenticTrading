...
| **Key custody** | The browser **never** calls FinSearch. ATL backend proxies via a new `/api/news/*` router; `FINGPT_API_KEY` stays server-side. | The endpoint is bearer-gated; shipping the key in `app.js` would publish it. Proxying also lets ATL cache (300s TTL, matching the producer's `Cache-Control: public, max-age=300`) so one browser reload doesn't fan out to the droplet. |
...
| **Adapter shape** | **One module, two consumers.** A shared fetch core (auth header, `?as_of`/`?tickers`, ETag revalidation, per-run memo) consumed by (a) `get_news_sentiment(universe, timestamp)` for backtest stepping and (b) the panel proxy for "latest". | The projection and error model are identical; only the artifact-selection key differs (`as_of=<step date>` vs. latest). Two modules would duplicate the fail-closed mapping. |
| **Panel refresh** | Fetch on Home show + re-poll every 5 min while visible (matching the server cache TTL); stop when hidden. | Data changes at most every 20 min; anything faster is theater. `home-page.js` already has the exact show/hide lifecycle hooks to copy. |
...
`dashboard/backend/integrations/news_sentiment.py` (new; `integrations/` gets an `__init__.py`):
...
Domain-boundary note: `integrations/` is a backend-root package (like `execution/`) — it may be imported by `execution/` and `api/`, and must not import either (keeps `test_architecture_boundaries.py` happy; verify the module isn't placed under `domain/`).
...
One route, one payload, one panel fetch. Caching via `cache.py` (new `CACHE_KEY_NEWS_SIGNALS`, `TTL_NEWS = 300`). Fail-closed: on any adapter failure the route returns `200` with `{"status": "unavailable", "feed": [], "signals": {}}` — the panel renders an honest empty state, never a 500.
...
- **Left — "Latest news"**: story rows (headline → source link, source name, relative time from `published`). Phase A: representative stories; Phase B: true raw feed. Every row's headline is an `<a>` to the story URL (**mandatory** — truth-infra requirement).
...
- Lifecycle: init/start on `onHomePageShow`, stop on `onHomePageHide` (copy `home-page.js` pattern); 5-min re-poll while visible; `AbortController` + timeout per the market-ticker convention (`app.js:1856-1857`).
...
- Adapter: projection field-by-field (incl. `rationale`, `age_hours` against a simulated step timestamp), every error-table row (401/503/400/404/degraded/network → correct fail-closed output + log level), memo/ETag behavior (second call same key → conditional).

"""
Paper Trading Module - session tracking and orchestration helpers.

The provider-specific Alpaca paper-account adapter (``Position``, ``Trade``,
``AlpacaPaperTradingClient``) moved to
``dashboard.backend.infrastructure.brokers.alpaca_paper`` in Phase 3C5A and is
re-exported here for backward compatibility. Session tracking
(``PaperTradingSession``, ``create_paper_trading_session``) remains here.
"""

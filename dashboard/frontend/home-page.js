...
function playHomeDemoRun(payload) {
    ...
    const windowEl = document.querySelector('.home-playground-window');
    const chat = document.getElementById('homePlaygroundChat');
    const panel = document.getElementById('homeDemoRunPanel');
    const title = document.getElementById('homePlaygroundTitleText');
    const kicker = document.getElementById('homeDemoRunKicker');
    const log = document.getElementById('homeDemoRunLog');
    if (windowEl) windowEl.classList.add('is-demo');
    if (chat) chat.hidden = true;
    if (panel) panel.hidden = false;
    if (title) title.textContent = `Hourly backtest · ${payload.model || 'GPT-5.5'}`;
    if (kicker) {
